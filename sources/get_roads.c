...
static int	remove_id_all_list(t_anthill *house, t_list_int **tab_list, int id)
{
	...
	if (id != -1)
	{
		...
		{
			if (tab_list[i]
				&& house->dijkstra->tab_rooms[id]->type != ROOM_START
				&& remove_id(&tab_list[i], id))
				rem = 1;
			if (tab_list[i]
				&& house->dijkstra->tab_rooms[id]->type == ROOM_START
				&& i == house->end->id
...
static void	init_roads(t_anthill *house, int explicit)
{
	...
	house->roads->nb_roads =
		(house->dijkstra->pred[house->end->id]->nb != -1) ? nb_roads : 0;
	house->roads->road =
		(t_list_int **)malloc(sizeof(t_list_int *) * (nb_roads + 1));
